############################################################################################################
##
##  CODE to replicate the results of the PLACEBO CHECKS in the paper:
...
# The code takes approximately [20] min to run. Differently from 'code.R', this script anticipates 
# artificially the intervention date (2019 instead of 2020) as we want to check whether the non-
# anticipation assumption holds in our setting: if the policy was not anticipated, there shouldn't
# be any significant causal effects.
...
# Removing 2020
ind <- which(dataset_full$year == 2020)
dataset_full <- dataset_full[-ind,]
# Fictional intervention date
int_date <- 2019
# Pre-intervention dataset
dataset_pre <- subset(dataset_full, year < int_date) 
 

# Comment: this is a 579 X 3 panel dataset (i.e., the number of units in the panel is 579
#          the number of times is 3). The dataset is organized in a a long manner
#          i.e., it has 579 X 3 = 1737 rows, one column for ID variable ('cod_sll2011'), 
...
### 1.1. Settings
mtry <- ((ncol(dataset_pre)-1)/3)
tunegrid <- expand.grid(.mtry=mtry)
ind <- which(colnames(dataset_pre) %in% c("year", "cod_sll2011"))

### 1.2. Random Forest
preliminary_rf <- train(punt_wle_medio_mat5 ~ .,
                        data = dataset_pre[ , -ind],
                        method = "rf",
                        metric = "RMSE",
...
#          greater than 20 (ind = 5) minimizes the RMSE of the predicted Y in the pre-intervention period and 
#          the best performing algorithm at this threshold is Gradient Boosting (check with best$method)
...
write.csv(ind_eff, file = paste0(wd, "/Output/placebo_individual_effects.csv"))
write.csv(ate_eff, file = paste0(wd, "/Output/placebo_ate.csv"))
